...
| 现象 | 原因/处理 |
|---|---|
| 测速只有 25G | 网络计划刚写入尚未生效：**重启两台（线插着）后 Run Test Again**；重启才是关键，仅反复插拔线缆不会让配置生效；若重启后仍低，再检查线缆型号 |
| 设备检查失败 | 系统未升级到 2026-04+；sudo 需免密或输入密码 |
| SSH 检查失败 | Sync 所在机器需与设备同网段且可 SSH |
| 端口显示 200G 但测速低 | 确认只有一根官方型号 QSFP112 DAC；重启后再测 |

> 本方案实测：首次测速只有 25G，期间尝试过反复插拔线缆均无效；**重启两台后**直接恢复到
> 200G+（后续 iperf3 实测两条链路各 ~107 Gbit/s，合计 ~214 Gbit/s）。因此若遇到 25G，
> 先别折腾线缆，直接重启（线保持插着），重启后重新 Run Test Again。
