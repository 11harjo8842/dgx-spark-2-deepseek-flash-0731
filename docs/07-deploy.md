...
- `DSPARK_MODEL=/cache/huggingface/models/DeepSeek-V4-Flash-0731`（本地路径，不经 HF hub）
...
脚本自动完成：解析 GID → 同步 compose/env 到 worker → 双端校验 compose → **先起 worker** →
起 head → 等待 API → 跑一次最小对话验证。
